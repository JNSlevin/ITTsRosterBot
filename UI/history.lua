...
local function renderHTags(data)
  ...
  if
    ((data.eventType == GUILD_EVENT_ITEM_SOLD or data.eventType == GUILD_EVENT_BANKITEM_REMOVED or data.eventType == GUILD_EVENT_BANKGOLD_REMOVED) and
      data.param1 ~= nil)
   then
    data.param1 = formatDisplayNameLink(data.param1)
  end
...
local function GuildHistoryManagerFilterScrollList(self)
  ...
  for i = 1, #self.masterList do
    ...
    if self.selectedSubcategory == nil or self.selectedSubcategory == data.subcategoryId then
      if
        (ITTsRosterBot.db.UI.history.gold == false and
       ...
       then
      elseif
        (ITTsRosterBot.db.UI.history.item == false and
       ...
       then
        ...
        if searchValue and string.len(searchValue) > 0 then
          if
            (data.param1 and ITTsRosterBot.Utils:HasValidHTag(searchValue, data.param1, 1)) or
